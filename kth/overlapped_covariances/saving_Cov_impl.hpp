...
OverlappedCovs_kth::calculate_covariances( field<string> in_all_people, int  in_dim  )
{
  ...
  for (int k = 0; k< parallel_names.n_rows; ++k)
  {
    ...
    one_video_multiple_covs(load_feat_video_i, load_labels_video_i, sc, pe, act );
    //getchar();
  }
...
inline
void
CovMeans_mat_kth::one_video_multiple_covs( std::string load_feat_video_i, std::string load_labels_video_i, int sc, int pe, int act )
{
  //   #pragma omp critical
  //   {
    ...
    int num_frames = labels(length_lab-1);
