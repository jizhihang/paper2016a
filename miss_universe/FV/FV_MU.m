...
for i = 1: length( all_years)
    ...
    svm_folder = 'svm_models'; 
     ...
     for k =1:length(vec_K)
         ...
         FV_MU_all_videos(path_dataset, path_features, view, all_years, K, dim, GMM_folder, FV_folder, run)
    ...
    for k =1:length(vec_K)
        ...
        for j = 1: length(vec_c)
            ...
            %Testing
            [predicted_output, accuracy, dec_values, labels_test]  = FV_test_rankSVM(path_dataset, view, years_test, K, dim, dim_FV, FV_folder, svm_folder, run);
